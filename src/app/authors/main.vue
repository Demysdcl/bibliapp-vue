  ...
  export default {
    ...
    */
    ...
    methods: {
      ...
      create() {
        this.$router.push({ name: 'authors.new', query: { page: this.currentPage } })
      },
      hide() {
        this.$router.push({ name: 'authors.index', query: { page: this.currentPage } })
      },
      /**
      * Brings actions from Vuex to the scope of
      * this component
      */
      ...mapActions(['authorsSetData', 'setFetching']),
      ...
      fetchPaginated() {
        ...
        */
        ...
        this.$http.get(`authors`).then(({ data }) => {
          ...
          */
          const currentPagination = createPagination(data, this.currentPage)
          this.authorsSetData({
            list: createPage(data, currentPagination, this.currentPage),
      ...
      * return a paginated set of authors
      ...
      */
      navigate(page) {
        ...
        */
        this.$router.push({ name: 'authors.index', query: { page } })
      },
      ...
      */
      askRemove(index) {
        ...
        swal({
          title: 'Are you sure?',
          text: `Author ${author.firstName} will be permanently removed.`,
          type: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#DD6B55',
          confirmButtonText: 'Yes, do it!',
          closeOnConfirm: false,
        }, () => this.remove(author)) // callback executed when OK is pressed
      },
      ...
      */
      remove(author) {
        this.$http.delete(`authors/${author.id}`).then(() => {
          /**
          * On success fetch a new set of Authors
          * based on current page number
          */
          this.fetchPaginated()

          /**
          * If we remove a author then
          ...
          */
          swal('Done!', 'Author removed.', 'success')
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.Authors.pagination,
      ...
      isFormVisible() {
        return this.$route.name === 'authors.new' || this.$route.name === 'authors.edit'
      },
    },
    /**
    ...
    */
    beforeRouteLeave(to, from, next) {
      this.$bus.$off('author.created')
      this.$bus.$off('author.updated')
      jQuery('body').off('keyup')
      next()
    },
    mounted() {
      /**
      * Author was created or updated, refresh the list
      */
      this.$bus.$on('author.created', this.fetch)
      this.$bus.$on('author.updated', this.fetch)
      /**
...
<template>
  <div>
    ...
    <!-- see /src/modules/authors/routes.js  -->
    ...
    <el-table v-loading.body="fetching" :data="authors" stripe border style="width: 100%">
